...
const FarmCardV3 = ({ farmInfo, chain, setSelectedFarm, walletAddress }) => {
  ...
  const stake = async () => {
    ...
    await tokenWeb3(farmInfo.lptoken, library.getSigner()).approve(
      farmInfo.address,
      parseEther(amountIn)
    );
    tokenWeb3(farmInfo.lptoken).once("Approval", async () => {
      const tx = await farmWeb3(farmInfo.address, library.getSigner()).deposit(
        parseEther(amountIn)
      );
      await tx.wait();
      ...
      >
            ...
            <Box>
              ...
              <Box sx={{ fontSize: "12px" }}>
                {farmInfo.start > new Date() && (
                  <Box sx={{ color: "skyBlue" }}>Farming is not started.</Box>
                )}
                {farmInfo.start < new Date() && farmInfo.end > new Date() && (
                  <Box sx={{ color: "skyBlue" }}>Farming is active.</Box>
                )}
                {farmInfo.bonusEndBlock < new Date() && (
                  <Box sx={{ color: "skyBlue" }}>Farming has finished.</Box>
                )}
              </Box>
            </Box>
          </Box>
          ...
          >
            <Grid container spacing={2}>
              <Grid item sm={10} md={2}>
                <DateRangeIcon
                  sx={{ color: "#1F8BED", mt: "4px", ml: "20px" }}
                />
              </Grid>
              <Hidden smDown>
        ...
        <Grid item xs={2}>
          ...
          >
            ...
            <Box sx={{ marginBottom: "4px" }}>{farmInfo.numFarmers}</Box>
          </Box>
        </Grid>
      </Grid>
      {/* stake */}
